...
This module can create fully functional session hosts. Consider using the [terraform-azurerm-avdsh](https://github.com/ColeHeard/terraform-azurerm-avdsh) companion module if  blue-green deployments or require more flexible personal VM scaling options.
...
| <a name="input_application_map"></a> [application\_map](#input\_application\_map) | A map of all applications and metadata. Required if var.pool\_type == application. | ```map(object({ app_name = string local_path = string cmd_argument = string aad_group = string }))``` | `null` | no |
...
| <a name="input_network_data"></a> [network\_data](#input\_network\_data) | The network data needed for sessionhost connectivity. | `any` | `null` | no |
| <a name="input_ou"></a> [ou](#input\_ou) | The OU a VM should be placed within. | `string` | `""` | no |
| <a name="input_pool_number"></a> [pool\_number](#input\_pool\_number) | The number of this pool. Use to avoid name collision. | `number` | n/a | yes |
...
| <a name="input_tags"></a> [tags](#input\_tags) | The tags for the virtual machines and their subresources. | `map(any)` | ```{ "Warning": "No tags" }``` | no |
...
| <a name="input_vmcount"></a> [vmcount](#input\_vmcount) | The number of VMs requested for this pool. | `number` | `0` | no |
...
| <a name="output_hp_output"></a> [hp\_output](#output\_hp\_output) | Hostpool information for consumption by an independent sessionhost module. |
